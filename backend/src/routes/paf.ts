...
export async function pafRoutes(app: FastifyInstance) { //
  ...
  // --- ALTERAÇÃO NO SCHEMA DE VALIDAÇÃO ---
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const newPaf = await prisma.paf.create({
          data: {
            ...data, // 'data' agora contém 'deadline' em vez de 'prazos'
            casoId: caseId, //
            autorId, //
          },
        })
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      try {
        const { caseId } = paramsSchema.parse(request.params) //
        // O schema de atualização agora espera 'deadline' (opcional)
        ...
        const updatedPaf = await prisma.paf.update({
          where: { casoId: caseId }, //
          data: {
            ...data, // 'data' agora contém 'deadline' (se foi enviado)
            updatedAt: new Date(), //
          },
        })
