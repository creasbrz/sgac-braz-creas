...
const defaultValues: CreateCaseFormData = {
  nomeCompleto: '', cpf: '', nascimento: '', sexo: '', telefone: '', endereco: '', urgencia: '', violacao: '', categoria: '', orgaoDemandante: '', agenteAcolhidaId: '', linkSei: '', observacoes: '', numeroSei: '', beneficios: [], origem: 'ESPONTANEA', dataEntrada: getLocalDateOnly(), 
}
...
export function CaseForm({ onCaseCreated, initialData, caseId }: CaseFormProps) {
  ...
  const { mutateAsync: submitCase, isPending } = useMutation({
    ...
    onError: (error) => {
      console.error(error)
      toast.error(getErrorMessage(error, isEditing ? 'Falha ao atualizar.' : 'Falha ao cadastrar.'))
    },
  })

  const onSubmit: SubmitHandler<CreateCaseFormData> = async (data) => await submitCase(data)

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={clsx("space-y-6", isPending && "opacity-50 pointer-events-none")}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border rounded-lg bg-card shadow-sm">
            <FormField control={form.control} name="nomeCompleto" render={({ field }) => (
                <FormItem className="lg:col-span-2"><FormLabel>Nome Completo</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
            ...
            <FormField control={form.control} name="cpf" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <MaskedInput mask={MASKS.CPF} placeholder="000.000.000-00" value={field.value || ''} onChange={field.onChange} onBlur={field.onBlur} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            )}/>
            ...
            <FormField control={form.control} name="telefone" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <MaskedInput mask={MASKS.PHONE} placeholder="(61) 90000-0000" value={field.value || ''} onChange={field.onChange} onBlur={field.onBlur} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            )}/>
            ...
            <FormField control={form.control} name="endereco" render={({ field }) => (
                <FormItem className="lg:col-span-3"><FormLabel>Endereço</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border rounded-lg bg-card shadow-sm">
            ...
            <FormField control={form.control} name="dataEntrada" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input type="date" {...field} readOnly={!isEditing} className={!isEditing ? "bg-muted opacity-70" : ""} /></FormControl>
                  <FormDescription>{isEditing ? "Cuidado ao alterar." : "Data atual."}</FormDescription>
                  <FormMessage />
                </FormItem>
            )}/>
            ...
            <FormField control={form.control} name="urgencia" render={({ field }) => (
                <FormItem className="lg:col-span-2">
                  <FormLabel>Urgência/Risco</FormLabel>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <FormControl><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger></FormControl>
            ...
            <FormField control={form.control} name="violacao" render={({ field }) => (
                <FormItem className="lg:col-span-2">
                  <FormLabel>Violação</FormLabel>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <FormControl><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger></FormControl>
                    <SelectContent>{LISTS.violacao.map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>
            )}/>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 border rounded-lg bg-card shadow-sm">
            <FormField control={form.control} name="orgaoDemandante" render={({ field }) => (
                <FormItem><FormLabel>Órgão Demandante</FormLabel><FormControl><Input {...field} placeholder="Ex: CRAS, Conselho Tutelar..." /></FormControl><FormMessage /></FormItem>
            )}/>

            <FormField control={form.control} name="numeroSei" render={({ field }) => (
                <FormItem>
                  <FormLabel>Número do SEI</FormLabel>
                  <FormControl>
                    <MaskedInput mask={MASKS.SEI} placeholder="00000-00000000/0000-00" value={field.value || ''} onChange={field.onChange} onBlur={field.onBlur} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            )}/>

            <FormField control={form.control} name="linkSei" render={({ field }) => (
                <FormItem><FormLabel>Link SEI</FormLabel><FormControl><Input type="url" {...field} value={field.value ?? ''} placeholder="https://..." /></FormControl><FormMessage /></FormItem>
            )}/>

            <FormField control={form.control} name="agenteAcolhidaId" render={({ field }) => (
                <FormItem className="lg:col-span-3">
                  <FormLabel>Agente Social (Triagem)</FormLabel>
                  <Select value={field.value} onValueChange={field.onChange} disabled={isLoadingAgents}>
                    <FormControl><SelectTrigger><SelectValue placeholder={isLoadingAgents ? "Carregando..." : "Selecione"} /></SelectTrigger></FormControl>
                    <SelectContent>
        ...
        <FormField control={form.control} name="observacoes" render={({ field }) => (
            <FormItem><FormLabel>Observações Gerais</FormLabel><FormControl><Textarea {...field} value={field.value ?? ''} className="min-h-[100px]" /></FormControl><FormMessage /></FormItem>
        )}/>
        ...
        <div className="flex justify-end pt-4 border-t">
          <Button type="submit" disabled={isPending} size="lg" className="w-full sm:w-auto min-w-[200px]">
            ...
            {isPending ? 'Salvando...' : (isEditing ? 'Atualizar Dados' : 'Cadastrar Caso')}
          </Button>
        </div>
      </form>
    </Form>
