...
import { z } from 'zod'
import { Loader2, Edit, History, PlusCircle, Printer } from 'lucide-react' // [ADD Printer]
...
import { generatePafPDF } from '@/utils/pdfGenerator' // [ADD Import]
...
function PafForm({ caseId, existingPaf, onClose }: { caseId: string, existingPaf: PafData | null, onClose: () => void }) {
  ...
  const { mutate: savePaf, isPending } = useMutation({
    mutationFn: async (data: PafFormData) => {
      ...
      return existingPaf ? api.put(`/cases/${caseId}/paf`, payload) : api.post(`/cases/${caseId}/paf`, payload)
    },
    onSuccess: () => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((d) => savePaf(d))} className="mt-4 space-y-4 animate-in fade-in">
        <FormField control={form.control} name="diagnostico" render={({ field }) => (
          <FormItem><FormLabel>Diagnóstico Sociofamiliar</FormLabel><FormControl><Textarea rows={3} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="objetivos" render={({ field }) => (
          <FormItem><FormLabel>Objetivos</FormLabel><FormControl><Textarea rows={3} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="estrategias" render={({ field }) => (
          <FormItem><FormLabel>Estratégias</FormLabel><FormControl><Textarea rows={3} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="deadline" render={({ field }) => (
          <FormItem className="max-w-xs"><FormLabel>Prazo para Reavaliação</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <div className="flex justify-end gap-2 pt-4 border-t">
          <Button type="button" variant="ghost" onClick={onClose} disabled={isPending}>Cancelar</Button>
          <Button type="submit" disabled={isPending}>{isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {existingPaf ? 'Salvar Nova Versão' : 'Criar PAF'}</Button>
        </div>
      </form>
    </Form>
  )
...
function DisplayPaf({ paf }: { paf: PafData }) {
  return (
    <div className="mt-4 space-y-6 animate-in fade-in">
      <DetailField label="Diagnóstico" value={paf.diagnostico} className="bg-muted/20 p-3 rounded-md border" />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <DetailField label="Objetivos" value={paf.objetivos} className="bg-muted/20 p-3 rounded-md border" />
        <DetailField label="Estratégias" value={paf.estrategias} className="bg-muted/20 p-3 rounded-md border" />
      </div>
      <DetailField label="Prazo Final para Reavaliação" value={formatDateSafe(paf.deadline)} labelClassName="text-amber-600 font-bold" />
      <div className="pt-4 border-t flex items-center justify-between text-xs text-muted-foreground">
        <span>Atualizado em {formatDateSafe(paf.createdAt)} (Versão {paf.versaoAtual})</span>
        <span>Autor: <strong className="text-foreground">{paf.autor.nome}</strong></span>
      </div>
    </div>
  )
}
    ...
    <div className="rounded-xl border bg-card text-card-foreground shadow-sm p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b pb-4">
        <h3 className="text-base font-semibold flex items-center gap-2">
          <span className="w-1 h-4 bg-purple-600 rounded-full" /> Plano de Acompanhamento (PAF)
        </h3>
          ...
          {paf && (
            <>
              ...
              <Button variant="outline" size="sm" onClick={() => setIsHistoryOpen(true)}>
                <History className="mr-2 h-3.5 w-3.5 text-muted-foreground" /> Histórico
              </Button>
            </>
          )}
          {canEditPaf && !isEditing && <Button variant="outline" size="sm" onClick={() => setIsEditing(true)}><Edit className="mr-2 h-3.5 w-3.5" /> Editar PAF</Button>}
          {canCreatePaf && !isEditing && <Button variant="default" size="sm" onClick={() => setIsEditing(true)}><PlusCircle className="mr-2 h-3.5 w-3.5" /> Elaborar PAF</Button>}
        </div>
      </div>

      {isLoading && <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>}
      {isError && <div className="p-4 bg-destructive/10 text-destructive rounded-md text-sm">Erro ao carregar PAF.</div>}
