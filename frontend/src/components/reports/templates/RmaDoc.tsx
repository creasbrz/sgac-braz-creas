...
// Certifique-se que esse arquivo existe, senão use o ReportLayout padrão ou remova e use <Page> direto
...
// Se ReportLayout e globalStyles não existirem no seu projeto,
// substitua por uma definição básica aqui para não quebrar:
const styles = StyleSheet.create({
  ...globalStyles, // Mantém globais se existirem
  // Adicione estilos básicos se necessário
  page: { padding: 30, fontSize: 10 },
  section: { marginBottom: 10, border: '1px solid #ccc' },
  sectionTitle: { fontSize: 12, fontWeight: 'bold', backgroundColor: '#eee', padding: 4 },
  table: { width: '100%' },
  row: { flexDirection: 'row', borderBottom: '1px solid #eee', alignItems: 'center', minHeight: 15 },
  cell: { padding: 2 },
  textCenter: { textAlign: 'center' },
  bold: { fontWeight: 'bold' },
  headerCell: { backgroundColor: '#f3f4f6', fontWeight: 'bold', fontSize: 8 }
});
...
// --- RESTO DO CÓDIGO (RowSimple, RowDemoStandard, etc.) IGUAL AO SEU ---
// Copie aqui os componentes RowSimple, RowDemoStandard, RowDemoChild, RowDemoLabor
// e o componente principal RmaDoc que você forneceu.
...
export const RmaDoc = ({ data }: { data: RmaReportData }) => {
  // Guard Clause contra undefined data
  if (!data || !data.bloco1) return <Document><Page><Text>Dados indisponíveis</Text></Page></Document>
