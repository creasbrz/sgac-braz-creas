...
/**
 * Valida um CPF brasileiro usando o algoritmo do dígito verificador.
...
/**
 * Schema de validação para o formulário de criação de um novo caso.
 */
export const createCaseFormSchema = z.object({
  ...
  telefone: z
    .string()
    .refine((tel) => tel.replace(/\D/g, '').length >= 10, {
...
/**
 * Schema de validação para o formulário de registo de uma nova evolução.
 */
...
/**
 * Schema de validação para o formulário de criação do PAF.
 */
...
/**
 * Schema de validação para o formulário de desligamento de um caso.
 */
export const closeCaseFormSchema = z.object({
  parecerFinal: z
    .string()
    .min(10, 'O parecer final deve ter no mínimo 10 caracteres.'),
  // Correção: A validação de "string não vazia" é feita com .min(1)
  motivoDesligamento: z
    .string()
